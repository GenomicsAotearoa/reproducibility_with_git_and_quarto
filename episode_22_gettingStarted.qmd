...
Now back in RStudio, click teh 'Render' button, which will simultaneously open up a Preview. You can change the Preview settings by clicking the settings button next to the Render button. The Preview can either be viewed in a new window or in a panel in RStudio. 
...
Hit `CTRL`{style="color:black"} + `C`{style="color:black"} on terminal to exit preview mode.

`quarto preview` is great for live viewing, but we always need to generate our final output with `quarto render`. More on rendering later in this workshop!
