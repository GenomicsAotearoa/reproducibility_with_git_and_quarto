...
First, make a new directory on your Desktop and call it `Learning-quarto`{style="color:blue"}. (**No spaces!**) 

Go to RStudio and click on `File > New Project > Existing Directory`{style="color:black"}, then use browse to select your `Learning-quarto`{style="color:blue"} folder. Click `Create Project.`{style="color:black"} 

![](images-quarto/quarto-setup-02.png){width=80%}
...
![New Quarto doc panel](images-quarto/quarto-setup-03.png)  

On the left hand side we can see three options: Document, Presentation, and Interactive, and for each of these we can see there are different outputs. We can change these outputs later, but for now, select Document and HTML (this is the recommended format for working in). Give this document a title, something like "Basic About Page". Before we create the document, we can see a couple of other options: **Engine** (leave this as Knitr), **Editor** (we will keep the visual editor box ticked), and then we can either click `Create`{style="color:black"} or `Create Empty Document`{style="color:black"}. 

For now, click on `Create`{style="color:black"}.
...
# Bonus! Callouts and Note boxes

We can also add "callouts" to our documents, which are boxes that can be used to highlight important information, tips, warnings, or notes.
To add a note callout, we use the following syntax:
...
You may realise that this text itself is in a callout!

Optionally you can add arguments such as collapse="true" and change the appearance to simple or minimal. Try a few of the following:
...
> You can also add information in a note, like this text here. Start your line with a <KBD>></KBD> and then add your note text. (Hint, you need a space between the <KBD>></KBD> and the text for this to work).

See more info on [Callout blocks here.](https://quarto.org/docs/authoring/callouts.html) 
...
Save your Quarto file (`CTRL`{style="color:black"} + `S`{style="color:black"} or `CMD`{style="color:black"} + `S`{style="color:black"}), give it a name if you have not already (*e.g.,* `getting-started.qmd`{style="color:blue"}) and save it into the directory you made earlier (mine is called `Learning-quarto`{style="color:blue"}).
...
::: {.callout-tip collapse="true"}
# Want to render and preview using a command line interface?
...
If you get a "commnand not found" output, [install the Quarto CLI from here.](https://quarto.org/docs/get-started/) It should automatically add quarto to your PATH. 
...
Hit <KBD>CTRL</KBD>  + <KBD>C</KBD> back in terminal to exit preview mode.
...
Quarto is a built-in tool within RStudio, so if rendering is not working you may need to update your R and RStudio to the latest version from posit.

If you do not have admin rights over your computer (*i.e.,* if you are using a University, Institutional or Organisation issued computer) you may need to first see if an update is available through your organisation's software management system. 

If you are still having an issue rendering, this may be because you need to install R markdown and dependancies.

Try running the following (this may take a few minutes to download all the packages)  
...
#### **Exercise:** Exploring revealjs and pdf (3 min)

1. Change the format of the output from `html` to `revealjs` and render.  Explore the revealjs (slideshow) format! 🤠   

> Hint: to see multiple slides, you need multiple headers. Add in new headings in the `.qmd`{style="color:blue"} document using either the Visual Editor to select and format text to Header 2,  or in the Source Editor type two # symbols then type the name of your header (*e.g.,* `## Header name`{style="color:black"}).   

2. Change the format of the output from `revealjs` to `typst` and render. Enjoy your new pdf! 🥳  

::: {.callout-note collapse="true"}
# PDF format

There are many ways to format your pdf using typst, including changing the font, adding in columns, even making a scientific poster. See the [Quarto docs on typst here](https://quarto.org/docs/output-formats/typst.html) for more options.  

Quarto can also render your documents into PDF using LaTeX, which you will need to install if you do not already have it.
