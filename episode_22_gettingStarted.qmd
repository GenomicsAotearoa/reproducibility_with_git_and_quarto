...
At the top of each .qmd document we create, we will need a YAML header. YAML (**Y**et **A**nother **M**arkup **L**anguage or **Y**AML **A**in't **M**arkup **L**anguage) is placed at the very top of the document and is used to define the document metadata and output settings. 

One thing to note about YAML headers: they are *fussy*. You must comply with the formatting rules such as indentations. 

### Document metadata

Title, author, date.

Why are we doing this? What impact does this have on reproducibility?

### Output format

Can output to a single format (e.g., HTML, PDF, word, slideJS) or multiple outputs (HTML + PDF).
...
Useful examples: code-fold. Mention other examples.
...
*Note:* In RMarkdown we call these 'code cells' and in Quarto they are referred to as code cells. 
...
- Controlling code cells: echo, eval, warnings, labels.