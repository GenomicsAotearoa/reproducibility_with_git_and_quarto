...
```{r}
round(mean(na.omit(penguins$bill_length_mm, na.rm = TRUE)),2)
...
```bash
The mean bill length of penguins in the dataset is "r round(mean(na.omit(penguins$bill_length_mm, na.rm = TRUE)),2)"mm.
```
