...
- Generate an `html`{style="color:blue"} document with well-documented code and embedded output.  
...
1. Create a new quarto document `(File > New File > Quarto Document)`{style="color:black"}, give it the title "Code in Quarto" and select `Create Empty Document`{style="color:black"}. Save the file as `code-in-quarto.qmd`{style="color:blue"}

2. We need a few packages for this episode. If you need to install these, you can do it now if you wish with the following code (you can run this directly in your console):  
...
RStudio (most common for Quarto workshops):
...
Mac: <KBD>Command</KBD> + <KBD>Shift</KBD> + <KBD>I</KBD>  
Windows/Linux: <KBD>Ctrl</KBD> + <KBD>Shift</KBD> + <KBD>I</KBD>  

#### EXERCISE 🧠🏋️‍♀️: Insert a code cell (2 min)
...
![](images-quarto/quarto-codecell-01.png)
...
For example, if you do not have the palmerpenguins package installed, this cell cannot successfully execute and the render process will fail. Using the `#`{style="color:green"} symbol within a code cell will cause that line of text to be 'commented out' (*i.e.,* it will be interpreted as plain text, rather than code). 
...
When `code-fold` is set to `true`, all code blocks will be automatically collapsed. This is useful when you want code to be available but would otherwise distract from the flow of the document *e.g.,* when teaching/training, presenting at lab meetings. 

The option `code-tools: true`, used in conjunction with `code-fold: true`, provides a simple button at the top of the html that can then be used to toggle all code to open/closed (an easy 'reset' of all code blocks). This also provides an option to see all the qmd source code.  

## Code cell options

Within each code cell we can add arguments that modify how the code is interpreted or displayed in the rendered document. We will add this information at the start of the code cell with `#|`{style="color:green"} (called the hash pipe). 
...
![](images-quarto/quarto-codecell-02.png)
...
![](images-quarto/quarto-codecell-03.png)
...
1. Open a code block.  
2. Directly under the header line of your code block, add the text `#| label: fig-penguinBills`.   
3. Then, add some `ggplot` code to make a figure. We'll do a simple `geom_point()` plot here using the palmerpenguins dataset as an example. Then, underneath your code block, write some text cross-referencing the figure label. See our examples below:
...
In the qmd file used to build the website you are looking at right now, the code block above contains two `#|`{style="color:green"} lines:
...
Then, the line `#| label: fig-penguinBills` is repeated, so that you can see it in the code block above. As it is separated from the start of the code block by a line, it gets read by r as a commented out line, the same as any line starting with a `#`{style="color:green"}. 

Below the code block the text reads:

See \@fig-penguinBills for a positive correlation between bill depth and bill length.

Here, the `@fig-penguinBills`{style="color:green"} automatically gets converted to the text "Figure 1", with a clickable link.

**Confused?** Here are pictures of the full code and output:

<u>Code behind the scenes:</u>

![](images-quarto/quarto-codeBTS-01.png)  


<u>Output:</u>

![](images-quarto/quarto-codeBTS-02.png)
...
For cross referencing tables it is very similar to figures, but instead we use the `tbl` prefix. We'll also add a Table caption using `#| tbl-cap:` (This works for figures too! Use `#| fig-cap`).   

1. Open a code block.    
2. Directly under the header line of your code block, add the text `#| label: tbl-penguin-mass-summary`  
3. On the next line also add a caption `#| tbl-cap: tbl-penguin-mass-summary` 
...
And cross referencing is done in a similar matter by using the label, e.g. using `@tbl-penguin-mass-summary`{style="color:green"} gives us the cross-reference @tbl-penguin-mass-summary, which is automatically converted to the words "Table 1" with the hyperlink. 

Now you try it out!

> Want more? For more ways to cross-reference, [see the Quarto docs here.](https://quarto.org/docs/authoring/cross-references.html)
...
#####  EXERCISE 🧠🏋️‍♀️: remove messages and warnings (2 min)
...
This hides the warning we saw before about the 2 rows containing missing values
...
This hides the messages from loading dplyr
...
> Note: You cannot use the same label twice within a document. 
...
For example, if we wanted to describe the mean bill length for the Adelie species of penguins in the penguins object, we could manually calculate the value:

```{r}
round(mean(na.omit(penguins$bill_length_mm)),2)
```

and then add a written statement:
...
2. Specify the language (in this case, R), immediately after the first backtick, then add a space.  

3. Include the code to be evaluated. 

**This is demonstrated below as inline code:**
...
**The same line of code above will render as:**
