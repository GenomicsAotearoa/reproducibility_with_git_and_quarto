...
* Tracking, adding, committing, restoring files.  
...
* Forking, rebasing, advanced merge strategies.  
* Extensive file recovery, log details and commit history (basics are covered).   
* `git reset` and `git revert` for undoing commits.     
...
* Further GitHub tools such as Releases and Actions.    
...
* Quarto books and more complex multi-document project types.    
...
See [Setup on local machine](setup.qmd) for setup instructions and checks. Git is run on the command line. For MacOS users you likely will not need to install anything extra for this to work. For Windows users you will need to download Git Bash. All learners will need to download RStudio, which comes with Quarto natively. Optionally you can download and install the [Quarto CLI](https://quarto.org/docs/get-started/), which will allow you to render Quarto documents without RStudio.
...
## {style="background: #D0C8E0; text-align:center;"}
