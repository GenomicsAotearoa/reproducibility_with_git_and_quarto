...
1.  Initiate a new, public github repo with a name like "Work-Portfolio" or equivalent (no spaces allowed in repo names). Copy the ssh line.  

2.  In RStudio, start a New Project, and use the Version Control option to link it to the new repo, pasting in the ssh line, and select where you want the directory to be (e.g., select 'Desktop', and your new directory will appear on your desktop) 

3.  Create three empty `.qmd`{style="color:blue"} files. For each file, set the YAML header to include an appropriate title and add at least one header and a piece of sample text to each document: 
    ...
    - `research_focus.qmd`{style="color:blue"}. Research focus will include a written statement about your focus as a researcher.
...
### Use RStudio Build to render website

Now is a good time to show a handy feature of having an RStudio project.  Note that this only works if you made an RStudio project first (we did this in 'Intial setup').

1. Save all your `.qmd`{style="color:blue"} files and `_quarto.yml`{style="color:blue"} file.

2. Close the RStudio project  
3. Reopen the RStudio project file (e.g., by clicking on the file `Work-portfolio.Rproj`{style="color:blue"}) to refresh the RStudio session.  
4. In the top right panel, there should now be a tab **Build**  
5. Click the Build tab, then click the button **Render website**. The qmd files don't need to be open in RStudio.  
...
### Refining the site: improving navigation, layout and features
...
Add a footer under the website section with: 

```bash
  page-footer:
    center:
      - text: "Made with love and Quarto"
...
Styles.css is stored in the top level of the directory. In the `_quarto.yml`{style="color:blue"} document under `format`, we can add a new line at the same indentation level as theme and added `css: styles.css` (remove the theme argument). 
...
First, save all your open `.qmd`{style="color:blue"} and `_quarto.yml`{style="color:blue"} files.  Then either:  

- Click the <KBD>Render website</KBD> button  
- Click the <KBD>Render</KBD> button on **every single `.qmd`{style="color:blue"} file.** This will create a `html`{style="color:blue"} file in `docs/`{style="color:blue"} for each qmd file.  


While in RStudio terminal, run:

```bash
git add . 
```
(The `.`{style="color:green"} adds everything in the current directory and subdirectories to the staging area)
...
> Hint: type `git push -u origin main` to set the upstream as origin main, next time you push you can type only `git push`)
...
You can also render and preview using the command line.

First, install [Quarto CLI tools](https://quarto.org/docs/get-started/). Type `quarto --version` to check.    

Commands to type in terminal:  

- `quarto render` - renders all qmd files if you have a quarto project.  
- `quarto render filename.qmd` - render only one file.  
- `quarto preview` - brings up a preview of your quarto project.   
- `quarto preview filename.qmd` - preview  only one file.  
