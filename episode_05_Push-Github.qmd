...
Go into the local `recipes`{style="color:blue"} repository, and run this command:  
...
Your output is going to look a little different depending on whether or not the `.ssh`{style="color:black"}  directory has ever been set up on the computer you are using and if you already have the SSH key pairs.  
...
*If you get this, proceed with step 3.1 Create an SSH key pair.*

##### Output option 2
If you **have set up SSH** before on your computer, but **do not have any ssh key pairs**, you will get an output something like this:

```
drwxr-xr-x  1 Username  Group   0 25 Sep  2025 ./
drwxr-xr-x  1 Username  Group   0 25 Sep  2025 ../
-rw-------@ 1 Username  Group  467 25 Sep  2025 config
```
*If you get this, proceed with step 3.1 Create an SSH key pair.*

##### Output option 3

If you **have set up SSH** before on your computer, **and you have also previously generated public and private key pairs** , you will get an output similar to below. You may have different files to these, but what we are looking for are the ssh keys. 

The file names are either `id_ed25519`{style="color:blue"}/`id_ed25519.pub`{style="color:blue"} or `id_rsa`{style="color:blue"}/`id_rsa.pub`{style="color:blue"} depending on how the key pairs were set up.  

```
drwx------  3 Username  Group    96B 25 Sep  2025 agent
-rw-------@ 1 Username  Group   467B 25 Sep  2025 config
-rw-------@ 1 Username  Group   419B 25 Sep  2025 id_ed25519
-rw-r--r--@ 1 Username  Group   110B 25 Sep  2025 id_ed25519.pub
-rw-------  1 Username  Group   3.9K 25 Sep  2025 known_hosts
-rw-------@ 1 Username  Group   3.2K 25 Sep  2025 known_hosts.old
drwxr-xr-x@ 2 Username  Group    64B 25 Sep  2025 sockets
```
*If you get this, you do not need to proceed with step 3.1 Create an SSH key pair. You can jump to step 3.2 Copy the public key to GitHub.* 
...
It will now ask you for a passphrase. For now, you may find it easier to not set one (just press <KBD>Enter</KBD>). 
...
The easiest way to set or remove the passphrase is to remake your keys.
...
```
After entering the same passphrase a second time (or press <KBD>Enter</KBD> again for no passphrase), we receive the confirmation:
...
::: {.callout-important}
# IMPORTANT! Only copy the PUBLIC key to GitHub. Never share your private key. 
:::

```bash
cat ~/.ssh/id_ed25519.pub
```
Copy the entire output. Avoid copying your prompt or other shell information. It should look something like this:

```
ssh-ed25519 AAAAC3NzaC1lZDl1NTE5AAAAlDmRA3d51X0uu9wXek559gfn6UFNF69yZjChyBlu2qKl your_email@example.com
```
...
Now, you can add the title (Use a title such as “My macbook pro laptop” so you can remember where the original key pair files are located), paste your public SSH key into the field, and click the “Add SSH key” to complete the setup.
...
You will most likely then see this message, with the words SOME-KEY-ID replaced by one of GitHubs SSH keys.  
...
This is a standard first-connection prompt that happens for any new host.

**You may want to be extra-safe and check the ssh key matches one of [GitHub's known public ssh keys listed here.](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/githubs-ssh-key-fingerprints)** 
...
(If you set a passphrase, you'll get asked to enter that now too)

Then you should get this success message:
...
Now that authentication is setup, we can return to the command line. This command will push all committed files from our **local repository** to the **repository on GitHub:**
...
> If you set up a passphrase, it will prompt you for it every time you push or pull. This is more secure, but can be a nuisance. Scroll back up to section 3.1 for instructions to set, remove, or avoid typing passphrases.

::: {.callout-note collapse="true"}
# ssh: connect to host github.com port 22: Connection timed out (or refused) error?

This can happen for some users due to a network block from their institution or workplace. 

You can try this fix.

Edit/create a config file like so:
...
This allows GitHub to let you use SSH over port 443 (the HTTPS port) to get around corporate firewalls.

After saving and exiting nano, try git push again.
...
Now go to your github and have a look - your new recipe repository with the `guacamole.md`{style="color:blue"} file should be online!
