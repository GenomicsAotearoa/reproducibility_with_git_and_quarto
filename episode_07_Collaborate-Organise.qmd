...
We can use `git fetch` to fetch information about files from the server (*i.e.,* origin/main) and compare to our local files (*i.e.,* main) to see if we need to pull down new copies of the files. In practice, its a good idea to *always* run `git pull` anyway before starting work for the day, to make sure you have the most up to date files. Git fetch will not change any of your local files.

#### Example 
***Demo only*** - *at this stage in the workshop you will not have any changes between your local copy and the copy on the server*:
...
The output shows there are lines that are different between our local **main** (a/guacamole.md) and our server **origin/main** (b/guacamole.md).  
...
`git diff` gives no output now - so all good to continue!
...
Before you create the issue, right click your recipes repo in the top left, and open it in a new tab. Then, click on `guacamole.md`{style="color:blue"} to bring up the code. On the left of the line you want to change, click to get the three dots to appear, click the dots, and click 'Copy permalink'. 
...
Our `guacamole.md`{style="color:blue"} file is not very big, so specifying the exact line is not really necessary, but if you have a file with hundreds of lines of code then being able to track and note exactly where typos are is really handy!

Now, go back to your terminal and open the file in `nano` to make the change you specified in your issue.
...
git commit -m "fix #2 specified number of avocados"
```
...
**Hint:** now try run `git fetch origin` then `git diff main origin/main` to see the changes you have made locally that are not yet on the server. The a/ file and b/ file are designated by order of the arguments in the `git diff` command. So, if you run `git diff origin/main main` instead, the a/ file will be the server version and the b/ file will be your local version. 
...
Go back to your GitHub repo and have a look at the commit message, you'll see the issue number is automatically linked. You can now click through and "Close issue" -- or you may see that the issue is automatically closed because you referenced it in the commit message with the [magic word **fix**!](https://docs.github.com/en/get-started/writing-on-github/working-with-advanced-formatting/using-keywords-in-issues-and-pull-requests)

### Ignoring files with .gitignore

`.gitignore` is a file that you can add to your repository to specify files or directories that you want Git to ignore. This is useful for files that are generated during the workflow, such as log files, temporary files, or large data files that you don't want to track in version control.
This way when you bulk add files with `git add .` you won't accidentally add files that you don't want to track. It is a text file and each line specifies which file/s or directories to ignore.
...
For more info, check out the [GitHub docs on .gitignore here](https://docs.github.com/en/get-started/git-basics/ignoring-files)
and the [software carpentry lesson here](https://swcarpentry.github.io/git-novice/06-ignore.html).
...
GitHub docs describe organsiations as *"... shared accounts where businesses and open-source projects can collaborate across many projects at once, with sophisticated security and administrative features."*   
