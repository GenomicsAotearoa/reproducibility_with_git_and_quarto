...
Have a look back at your main repo now. You'll see the commit message has updated to the most recent one you used when you committed the changes to `guacamole.md`{style="color:blue"} on the new branch.
...
Merge conflicts happen when the remote repository has changes that have not been incorporated into the local repository. Git will reject the push/pull if it detects merge conflicts, and you may then need to tell Git how to resolve these. Further reading on merge conflicts in the [Git documentation here.](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/addressing-merge-conflicts/about-merge-conflicts)

Let's create a merge conflict here and see how we can resolve it with git. 

After we merged our `test-new-recipe`{style="color:darkorange"} branch into `main`{style="color:darkorange"}, we now have a situation where the `main`{style="color:darkorange"} branch has changes that are not in our local repository. If we try to push or pull any changes we make to `guacamole.md`{style="color:blue"} now, we will get an error message about merge conflicts.

First, let's swap to `main`{style="color:darkorange"} branch and make a change to `guacamole.md`{style="color:blue"}:
...
![](images_git/conflict.svg)

:::


Let's try pull down the remote version.
...
# Did you get a different output to the above?

The above is the newer git output -- the merge strategy is not built in and allows you to chose how you want to merge.

In older git versions (before Git 2.27.0), the merge strategy defaults to merging the upstream branch into your local branch automatically.  
...
You can check your git version with `git --version` and see the [setup](setup.qmd) page for instructions to update git.  
:::
...
From github.com:username/recipes
 * branch            main       -> FETCH_HEAD
Auto-merging recipes/guacamole.md
CONFLICT (content): Merge conflict in recipes/guacamole.md
Automatic merge failed; fix conflicts and then commit the result.
```
